...
def mission_phlc(leader_cf, follower_cf, code, leader_pos, follower_pos):
    ...
    # Create threads for landing
    leader_land_thread = threading.Thread(target=leader_phlc.land)
    follower_land_thread = threading.Thread(target=follower_phlc.land)

    # Start landing both drones
    leader_land_thread.start()
    follower_land_thread.start()

    # Wait until both drones have landed
    leader_land_thread.join()
    follower_land_thread.join()
