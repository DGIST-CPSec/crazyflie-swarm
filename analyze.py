...
ax[0,0].plot(df['timestamp'], -2.6*np.ones(len(df['timestamp'])), 'r--')
ax[0,0].plot(df['timestamp'],  2.6*np.ones(len(df['timestamp'])), 'r--')
ax[0,0].set_ylim([-2.6, 2.6])
ax[0,0].set_title('x')
ax[1,0].plot(df['timestamp'], df['y'])
ax[1,0].plot(df['timestamp'], -2.6*np.ones(len(df['timestamp'])), 'r--')
ax[1,0].plot(df['timestamp'],  2.6*np.ones(len(df['timestamp'])), 'r--')
ax[1,0].set_ylim([-2.6, 2.6])
ax[1,0].set_title('y')
ax[2,0].plot(df['timestamp'], df['z'])
ax[2,0].plot(df['timestamp'], -2.6*np.ones(len(df['timestamp'])), 'r--')
ax[2,0].plot(df['timestamp'],  2.6*np.ones(len(df['timestamp'])), 'r--')
